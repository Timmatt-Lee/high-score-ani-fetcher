...
def test_action_routes_missing_data(client: Any) -> None:
    ...
    assert (
        client.post(
            "/add_to_favorites", headers={"Accept": "application/json"}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/move_to_trash", headers={"Accept": "application/json"}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/restore_from_trash", headers={"Accept": "application/json"}
        ).status_code
        == 400
    )
...
def test_make_request_with_retry(mock_get: Any) -> None:
    ...
    assert (
        server._make_request_with_retry(
            "http://test", {}, retry_count=2, min_delay=0, max_delay=0
        )
        == mock_resp
    )
    ...
    assert (
        server._make_request_with_retry(
            "http://test", {}, retry_count=1, min_delay=0, max_delay=0
        )
        is None
    )
...
def test_scrape_anime_details(mock_make_req: Any) -> None:
    ...
    mock_make_req.return_value = mock_resp
    assert server.scrape_anime_details("http://test") == (
        9.5,
        1000,
        "Test Description...",
    )
...
def test_parse_card_basic_info() -> None:
    ...
    assert (
        info["watch_count"] == 0
    )  # It actually fails to parse because first <p> is title
    assert info["episode_count"] == "12"
    assert info["upload_date"] == "2023"

    assert (
        server._parse_card_basic_info(BeautifulSoup("<a></a>", "html.parser").find("a"))
        is None
    )
