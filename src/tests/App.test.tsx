  ...
  storage: {
    local: {
      get: vi.fn(async (keys: string[]) => {
        ...
        keys.forEach(k => { if (storageMock[k]) result[k] = storageMock[k] })
        return result
      }),
...
describe('App rendering', () => {
  ...
  it('handles chrome.storage.get error gracefully', async () => {
    vi.stubGlobal('chrome', {
      storage: { local: { get: vi.fn().mockRejectedValue(new Error('storage error')), set: vi.fn() } }
    })
    await act(async () => { render(<App />) })
    expect(screen.getByText('No anime found in this list.')).toBeDefined()
  })
...
describe('Tab switching', () => {
  it('switches to Favorites tab', async () => {
    const fav = makeAnime({ title: 'Fav Anime', link: 'https://ani.gamer.com.tw/anime.php?sn=2' })
  ...
  it('switches to Trash tab', async () => {
    const trashItem = makeAnime({ title: 'Trash Anime', link: 'https://ani.gamer.com.tw/anime.php?sn=3' })
...
describe('Card actions', () => {
  ...
  it('moves item to Favorites and saves via localStorage when chrome undefined', async () => {
    vi.stubGlobal('chrome', undefined)
    localStorage.setItem('animeData', JSON.stringify({ searchList: [makeAnime()], favorites: [], trash: [] }))
    await act(async () => { render(<App />) })
    fireEvent.click(screen.getByText('❤ Favorite'))
...
describe('Scan functionality', () => {
  it('shows Scanning... and progress bar while running', async () => {
    ...
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockReturnValue(
      new Promise<AnimeItem[]>(resolve => { resolveScrape = () => resolve([]) })
    )
  ...
  it('filters results by score threshold after scan and fires progress callback', async () => {
    const highScore = makeAnime({ title: 'High Score', score: 5.0, episode_count: '12' })
    const lowScore = makeAnime({ title: 'Low Score', score: 4.0, episode_count: '12', link: 'http://other' })
    ...
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockImplementation(async (link) => {
      if (link === highScore.link) return { score: 5.0, rating_count: 100, description: 'Good' }
      return { score: 4.0, rating_count: 10, description: 'Meh' }
    })

    await act(async () => { render(<App />) })
    await act(async () => { fireEvent.click(screen.getByText('Scan Bahamut')) })
  ...
  it('skips items with less than 10 episodes', async () => {
    const shortShow = makeAnime({ title: 'Short Show', episode_count: '5', score: 9.0 })
    vi.spyOn(ScraperService, 'getTotalPages').mockResolvedValue(1)
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockResolvedValue([shortShow])
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockResolvedValue({ score: 9.0, rating_count: 100, description: 'x' })
  ...
  it('skips OVA titles', async () => {
    const ova = makeAnime({ title: 'Great Show OVA Special', episode_count: '12', score: 9.0 })
    vi.spyOn(ScraperService, 'getTotalPages').mockResolvedValue(1)
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockResolvedValue([ova])
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockResolvedValue({ score: 9.0, rating_count: 100, description: 'x' })
  ...
  it('skips items already in trash', async () => {
    const trashItem = makeAnime({ title: 'In Trash', episode_count: '12', score: 9.0 })
    storageMock['trash'] = [trashItem]
    vi.spyOn(ScraperService, 'getTotalPages').mockResolvedValue(1)
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockResolvedValue([trashItem])
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockResolvedValue({ score: 9.0, rating_count: 100, description: 'x' })
  ...
  it('skips items already in favorites', async () => {
    const favItem = makeAnime({ title: 'In Fav', episode_count: '12', score: 9.0 })
    storageMock['favorites'] = [favItem]
    vi.spyOn(ScraperService, 'getTotalPages').mockResolvedValue(1)
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockResolvedValue([favItem])
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockResolvedValue({ score: 9.0, rating_count: 100, description: 'x' })
  ...
  it('handles non-numeric episode count by skipping item', async () => {
    const naEp = makeAnime({ title: 'NA Ep', episode_count: 'N/A', score: 9.0 })
    vi.spyOn(ScraperService, 'getTotalPages').mockResolvedValue(1)
    vi.spyOn(ScraperService, 'fetchAllWithConcurrency').mockResolvedValue([naEp])
    vi.spyOn(ScraperService, 'scrapeAnimeDetails').mockResolvedValue({ score: 9.0, rating_count: 100, description: 'x' })
