    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.warn(
            `Failed to retrieve text snippet from non-ok response: ${errMsg}`,
          );
          ...
          // stream has already been read or if the connection was aborted), we fall back to an empty snippet
