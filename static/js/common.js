...
$(document).on("click", ".single-action-button", function (event) {
    ...
    if (!actionUrl || !animeLink) {
        console.error("Missing data-action-url or data-anime-link on button:", button[0]);
        showToast("Could not perform action: button configuration missing.", "error");
        return;
    }
    ...
    fetch(actionUrl, {
        ...
        if (!response.ok) {
            ...
            return response
                ...
                .catch(() => null)
                .then((errData) => {
                const errorMsg = (errData === null || errData === void 0 ? void 0 : errData.error) ||
        ...
        if ($row.length > 0) {
            $row.fadeOut(300, function () {
                ...
                if (tableBody.find("tr[data-link]").length === 0 &&
                    $emptyRow.length > 0) {
                    $emptyRow
                        .find("td")
...
function showToast(message, type = "success") {
    ...
    actionToastElement.classList.remove("bg-success", "bg-danger", "bg-warning", "text-white", "text-dark");
...
function formatLargeNumberJS(num) {
    ...
    try {
        ...
        if (isNaN(num))
            return originalNumStr; // Return original if still not number
        num = Number(num);
        if (num === 0)
            return 0;
        ...
        if (formatted.endsWith(".0")) {
            ...
            return (tempNum.toLocaleString(undefined, {
...
function performGenericBatchAction(endpointUrl, tableId, actionType) {
    ...
    const tableBody = tableElement.find("tbody");
    const selectedCheckboxes = tableBody.find("input[name='selected_anime']:checked"); // Get checked checkboxes
