...
function connectSSE() {
  ...
  eventSource.onmessage = function(event) {
      try {
          ...
          if (payload.type === 'progress') {
               ...
               loadingDetails.textContent = `Processed: ${data.loaded_count} / Est. ${data.total_estimated} | Current: ${data.current_anime || 'N/A'}`;
               statusMessage.textContent = data.status_message || '';
               statusMessage.className = data.status_message && data.status_message.toLowerCase().includes('retry') ? 'mt-1 small retry' : 'mt-1 small';
          ...
          // Add/Update Table Row
              ...
              const safeLinkSelector = `tr[data-link="${anime.link.replace(/"/g, '\\"')}"]`;
              ...
              if ($row.length > 0) { // UPDATE
                  let cells = $row.find('td');
                  cells.eq(1).html(`<a href="${anime.link}" target="_blank" title="${anime.title}">${anime.title}</a>`);
          ...
          // Handle Final State
               ...
               if (data.status_message.includes("finished")) { statusMessage.textContent = "Scraping finished!"; }
               else if (data.status_message.includes("stopped")) { statusMessage.textContent = "Scraping stopped."; }
               else { statusMessage.textContent = data.status_message || "Scraping ended."; }

               if ($animeTableBody.find('tr[data-link]').length === 0) {
                    if ($('#empty-row').length > 0) { $('#empty-row').show(); }
                    else { $animeTableBody.append(`<tr id="empty-row"><td colspan="9" class="text-center text-muted">No anime found matching the criteria (Score > ${scoreThreshold} and not OVA).</td></tr>`);} // Use scoreThreshold var
               } else { $emptyRow.hide(); }

               if (eventSource) { eventSource.close(); console.log("SSE closed by client."); }
          }
      } catch (e) { console.error("Error processing SSE:", e, "Raw:", event.data); }
  }; // end onmessage
  ...
  eventSource.onerror = function(error) {
      ...
      if(statusMessage) statusMessage.textContent = 'Progress connection lost. Refresh or try starting again.';
...
$(function () {
  ...
  $("#select-all").on("change", function() {
      ...
      $animeTableBody.find("input[name='selected_anime']").prop("checked", isChecked);
  });
  ...
  if (startScrapeBtn) {
      startScrapeBtn.addEventListener('click', () => {
          ...
          fetch('/start_scrape').then(response => { // Assuming /start_scrape route
              if (!response.ok) { startScrapeBtn.disabled = false; if(statusMessage) statusMessage.textContent = "Failed to start."; }
              else { console.log("Scraping start request sent..."); connectSSE(); }
          }).catch(err => { startScrapeBtn.disabled = false; if(statusMessage) statusMessage.textContent = "Error: " + err; });
      });
  }
  if (stopScrapeBtn) {
      stopScrapeBtn.addEventListener('click', () => {
           stopScrapeBtn.disabled = true;
            fetch('/stop_scrape').then(response => { // Assuming /stop_scrape route
                if (!response.ok) { if(statusMessage) statusMessage.textContent = "Failed to send stop.";}
                else { console.log("Stop request sent..."); if(statusMessage) statusMessage.textContent = "Stop requested..."; }
            }).catch(err => { if(statusMessage) statusMessage.textContent = "Error stopping: " + err; });
      });
  }

  // Initial State Logic (Uses 'initialScrapingState' defined in the inline script)
  // Ensure all elements accessed here exist
  if (typeof initialScrapingState !== 'undefined' && initialScrapingState.is_running) {
      if(loadingProgress) loadingProgress.style.display = 'flex';
      if(startScrapeBtn) startScrapeBtn.disabled = true;
      if(stopScrapeBtn) stopScrapeBtn.disabled = initialScrapingState.stop_requested;

      const initialProgress = initialScrapingState.progress;
      if(initialProgress && progressBar && progressText && loadingDetails && statusMessage) {
           const percentage = (initialProgress.percentage || 0).toFixed(1);
           progressBar.style.width = percentage + '%';
           progressBar.setAttribute('aria-valuenow', percentage);
           progressText.textContent = `${percentage}%`;
           loadingDetails.textContent = `Processed: ${initialProgress.loaded_count || 0} / Est. ${initialProgress.total_estimated || 0} | Current: ${initialProgress.current_anime || 'N/A'}`;
           statusMessage.textContent = initialProgress.status_message || 'Scraping in progress...';
           statusMessage.className = initialProgress.status_message && initialProgress.status_message.toLowerCase().includes('retry') ? 'mt-1 small retry' : 'mt-1 small';
           progressBar.classList.add('progress-bar-animated');
       }
       $emptyRow.hide();
       connectSSE(); // Reconnect SSE
  } else { // Not running initially
      if(loadingProgress) loadingProgress.style.display = 'none';
      if(startScrapeBtn) startScrapeBtn.disabled = false;
      if(stopScrapeBtn) stopScrapeBtn.disabled = true;
