    ...
    eventSource.onmessage = function (event) {
        try {
            ...
            // --- Add/Update Table Row ---
                ...
                const $row = $animeTableBody.find(safeLinkSelector);
                ...
                if ($row.length > 0) {
                    ...
                    cells
                        .eq(1)
                        .html(`<a href="${anime.link}" target="_blank" title="${anime.title}">${anime.title}</a>`);
            ...
            // Handle Final State
                ...
                if ($animeTableBody.find("tr[data-link]").length === 0) {
                    if ($("#empty-row").length > 0) {
                        ...
                        $animeTableBody.append(`<tr id="empty-row"><td colspan="9" class="text-center text-muted">No anime found matching the criteria (Score > ${scoreThreshold} and not OVA).</td></tr>`);
...
$(function () {
        ...
        startScrapeBtn.addEventListener("click", () => {
            ...
            fetch("/start_scrape")
                ...
                if (!response.ok) {
                    startScrapeBtn.disabled = false;
                    if (statusMessage)
                        statusMessage.textContent = "Failed to start.";
                }
                else {
        ...
        stopScrapeBtn.addEventListener("click", () => {
            ...
            fetch("/stop_scrape")
                ...
                if (!response.ok) {
                    ...
                    if (statusMessage)
                        statusMessage.textContent = "Stop requested...";
                }
            })
                .catch((err) => {
                if (statusMessage)
    ...
    if (typeof initialScrapingState !== "undefined" &&
